...
class storelocator {
    ...
    isochrone = {
        url: 'https://{azMapsDomain}/route/range/json?api-version=1.0&query={query}&traffic={traffic}&travelMode={travelMode}&timeBudgetInSec={timeBudgetInSec}&vehicleMaxSpeed={vehicleMaxSpeed}&avoid={avoid}',
    ...
    */
    async drawWalkingIsochrone(position) {
        const requestUrl = this.isochrone.url
            ...
            .replace('{travelMode}', 'car') // 'pedestrian' is not supported in the isochrone API.
            .replace('{timeBudgetInSec}', this.options.walkingTime * 60)
            .replace('{vehicleMaxSpeed}', 5)
            .replace('{avoid}', 'motorways');
